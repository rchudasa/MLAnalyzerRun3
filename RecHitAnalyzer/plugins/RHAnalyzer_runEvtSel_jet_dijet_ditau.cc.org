...
bool RecHitAnalyzer::runEvtSel_jet_dijet_ditau( const edm::Event& iEvent, const edm::EventSetup& iSetup )
{
  ...
  // comment out for now, used for svfit
  ...
  for ( unsigned int j = 0; j < vMatchedRecoJetIdxs.size(); j++ ) {
    ...
    pat::Tau iTau1( taus, vMatchedRecoTauIdxs[j] );
  ...
  for ( unsigned int j = 0; j < vJetTau.size()-1; j++ ) {
    pat::Tau iTau1( taus, vJetTau[j].recoTauIdxs );
    pat::Jet iJet1( jets, vJetTau[j].recoJetIdxs );

    for ( unsigned int k = j+1; k < vJetTau.size(); k++ ) {

      if ( k <= j ) continue;
      pat::Tau iTau2( taus, vJetTau[k].recoTauIdxs );
      pat::Jet iJet2( jets, vJetTau[k].recoJetIdxs );
	...
	// comment out for now, used in svfit
	/*
...
*/
  ...
  for ( unsigned int j = 0; j < vJetTauCut.size(); j++ ) {
    ...
    pat::Tau iTau1( taus, vJetTauCut[j].tauIdxs );
    pat::Jet iJet1( jets, vJetTauCut[j].jetIdxs );
  ...
  for ( unsigned int j = 0; j < vJetTauFrameCropped.size()-1; j++ ) {
    pat::Tau iTau1( taus, vJetTauFrameCropped[j].tauIdxs );
    pat::Jet iJet1( jets, vJetTauFrameCropped[j].jetIdxs );


    for ( unsigned int k = j+1; k < vJetTauFrameCropped.size(); k++ ) {

      if ( k <= j ) continue;
      pat::Tau iTau2( taus, vJetTauFrameCropped[k].tauIdxs );
      pat::Jet iJet2( jets, vJetTauFrameCropped[k].jetIdxs );
