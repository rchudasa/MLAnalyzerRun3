...
TH2F *hEvt_EE_energy_miniAOD[nEE];
TProfile2D *hECAL_energy_miniAOD;
std::vector<float> vECAL_energy_miniAOD_;

// Initialize branches _______________________________________________________________//
void RecHitAnalyzer::branchesECALstitchedminiAOD ( TTree* tree, edm::Service<TFileService> &fs ) {

  // Branches for images
  tree->Branch("ECAL_energy_miniAOD",    &vECAL_energy_miniAOD_);
  // Intermediate helper histogram (single event only)
  hEvt_EE_energy_miniAOD[0] = new TH2F("evt_EEm_energy_miniAOD", "E(i#phi,i#eta);i#phi;i#eta",
      EB_IPHI_MAX, -TMath::Pi(), TMath::Pi(),
      5*(HBHE_IETA_MAX_HE-1-HBHE_IETA_MAX_EB), eta_bins_EEm );
  hEvt_EE_energy_miniAOD[1] = new TH2F("evt_EEp_energy_miniAOD", "E(i#phi,i#eta);i#phi;i#eta",
      EB_IPHI_MAX, -TMath::Pi(), TMath::Pi(),
      5*(HBHE_IETA_MAX_HE-1-HBHE_IETA_MAX_EB), eta_bins_EEp );

  // Histograms for monitoring
  hECAL_energy_miniAOD = fs->make<TProfile2D>("ECAL_energy_miniAOD", "E(i#phi,i#eta);i#phi;i#eta",
...
void fillECAL_with_EEprojminiAOD ( TH2F *hEvt_EE_energy_miniAOD_, int ieta_global_offset, int ieta_signed_offset ) {

  int ieta_global_, ieta_signed_;
  int ieta_, iphi_, idx_;
  float energy_;

  for (int ieta = 1; ieta < hEvt_EE_energy_miniAOD_->GetNbinsY()+1; ieta++) {
    ieta_ = ieta - 1;
    ieta_global_ = ieta_ + ieta_global_offset;
    ieta_signed_ = ieta_ + ieta_signed_offset;
    for (int iphi = 1; iphi < hEvt_EE_energy_miniAOD_->GetNbinsX()+1; iphi++) {

      energy_ = hEvt_EE_energy_miniAOD_->GetBinContent( iphi, ieta );
      ...
      vECAL_energy_miniAOD_[idx_] = energy_;
      // Fill histogram for monitoring
      hECAL_energy_miniAOD->Fill( iphi_, ieta_signed_, energy_ );
...
void RecHitAnalyzer::fillECALstitchedminiAOD ( const edm::Event& iEvent, const edm::EventSetup& iSetup ) {
  ...
  vECAL_energy_miniAOD_.assign( 2*ECAL_IETA_MAX_EXT*EB_IPHI_MAX, 0. );
  for ( int iz(0); iz < nEE; ++iz ) hEvt_EE_energy_miniAOD[iz]->Reset();
  ...
  for ( EcalRecHitCollection::const_iterator iRHit = EERecHitsH_->begin();
    ...
    hEvt_EE_energy_miniAOD[iz_]->Fill( phi, eta, iRHit->energy() );
  ...
  fillECAL_with_EEprojminiAOD( hEvt_EE_energy_miniAOD[0], ieta_global_offset, ieta_signed_offset );
  ...
  for ( EcalRecHitCollection::const_iterator iRHit = EBRecHitsH_->begin();
    ...
    vECAL_energy_miniAOD_[idx_] = energy_;
    // Fill histogram for monitoring
    hECAL_energy_miniAOD->Fill( iphi_, ieta_signed, energy_ );
  ...
  fillECAL_with_EEprojminiAOD( hEvt_EE_energy_miniAOD[1], ieta_global_offset, ieta_signed_offset );
