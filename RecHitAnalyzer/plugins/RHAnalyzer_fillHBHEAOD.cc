...
TH2F *hEvt_HBHE_energy_AOD;
TProfile2D *hHBHE_energy_EB_AOD;
TProfile2D *hHBHE_energy_AOD;
std::vector<float> vHBHE_energy_EB_AOD_;
std::vector<float> vHBHE_energy_AOD_;

// Initialize branches _______________________________________________________//
void RecHitAnalyzer::branchesHBHEAOD ( TTree* tree, edm::Service<TFileService> &fs ) {

  // Branches for images
  tree->Branch("HBHE_energy_EB_AOD", &vHBHE_energy_EB_AOD_);
  tree->Branch("HBHE_energy_AOD",    &vHBHE_energy_AOD_);
  // Intermediate helper histogram (single event only)
  hEvt_HBHE_energy_AOD = new TH2F("evt_HBHE_energy_AOD", "E(i#phi,i#eta);i#phi;i#eta",
      HBHE_IPHI_NUM,           HBHE_IPHI_MIN-1,    HBHE_IPHI_MAX,
      2*(HBHE_IETA_MAX_HE-1),-(HBHE_IETA_MAX_HE-1),HBHE_IETA_MAX_HE-1 );

  // Histograms for monitoring
  hHBHE_energy_AOD = fs->make<TProfile2D>("HBHE_energy_AOD", "E(i#phi,i#eta);i#phi;i#eta",
      HBHE_IPHI_NUM,      HBHE_IPHI_MIN-1, HBHE_IPHI_MAX,
      2*HBHE_IETA_MAX_HE,-HBHE_IETA_MAX_HE,HBHE_IETA_MAX_HE );
  hHBHE_energy_EB_AOD = fs->make<TProfile2D>("HBHE_energy_EB_AOD", "E(i#phi,i#eta);i#phi;i#eta",
      HBHE_IPHI_NUM, HBHE_IPHI_MIN-1,HBHE_IPHI_MAX,
...
void RecHitAnalyzer::fillHBHEAOD ( const edm::Event& iEvent, const edm::EventSetup& iSetup ) {

  int iphi_, ieta_, ietaAbs_, idx_;
  float energy_;
  //float eta, GlobalPoint pos;

  vHBHE_energy_EB_AOD_.assign( 2*HBHE_IPHI_NUM*HBHE_IETA_MAX_EB, 0. );
  vHBHE_energy_AOD_.assign( 2*HBHE_IPHI_NUM*(HBHE_IETA_MAX_HE-1), 0. );
  hEvt_HBHE_energy_AOD->Reset();

  edm::Handle<HBHERecHitCollection> HBHERecHitsH_;
  iEvent.getByToken( HBHERecHitAODCollectionT_, HBHERecHitsH_ );
  ...
  */
  ...
  for ( HBHERecHitCollection::const_iterator iRHit = HBHERecHitsH_->begin();
    ...
    if ( hId.ietaAbs() > HBHE_IETA_MAX_FINE ) {
      ...
      hHBHE_energy_AOD->Fill( iphi_  , ieta_, energy_*0.5 );
      hHBHE_energy_AOD->Fill( iphi_+1, ieta_, energy_*0.5 );
      // Fill intermediate helper histogram
      hEvt_HBHE_energy_AOD->Fill( iphi_  , ieta_, energy_*0.5 );
      hEvt_HBHE_energy_AOD->Fill( iphi_+1, ieta_, energy_*0.5 );
      continue;
    }
    ...
    // Fill histograms normally
      ...
      hHBHE_energy_AOD->Fill( iphi_,ieta_,energy_ );
      hEvt_HBHE_energy_AOD->Fill( iphi_,ieta_,energy_ );
    }
    ...
    // Fill histograms for monitoring
    hHBHE_energy_EB_AOD->Fill( iphi_,ieta_,energy_ );
    ...
    vHBHE_energy_EB_AOD_[idx_] += energy_;
  ...
  for (int ieta = 1; ieta < hEvt_HBHE_energy_AOD->GetNbinsY()+1; ieta++) {
    for (int iphi = 1; iphi < hEvt_HBHE_energy_AOD->GetNbinsX()+1; iphi++) {

      energy_ = hEvt_HBHE_energy_AOD->GetBinContent( iphi, ieta );
      //if(energy_>0.0) std::cout << "HB HE rec hit energy:" << energy_ << std::endl;
      if ( energy_ <= zs ) continue;
      idx_ = (ieta-1)*HBHE_IPHI_NUM + (iphi-1);
      // Fill vector for image
      vHBHE_energy_AOD_[idx_] = energy_;
